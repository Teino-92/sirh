...
class LeaveRequestsController < ApplicationController
  ...
  def index
    ...
    case params[:filter]
    when 'upcoming'
      @leave_requests = policy_scope(LeaveRequest).approved.where('start_date >= ?', Date.current).order(:start_date).includes(:approved_by)
    when 'history'
      @leave_requests = policy_scope(LeaveRequest).where('end_date < ?', Date.current).order(start_date: :desc).includes(:approved_by)
    else
      @leave_requests = policy_scope(LeaveRequest).order(created_at: :desc).includes(:approved_by)
    end
  end
